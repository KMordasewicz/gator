...
const getURLFeeds = `-- name: GetURLFeeds :one
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds
WHERE url = $1
`

func (q *Queries) GetURLFeeds(ctx context.Context, url string) (Feed, error) {
	row := q.db.QueryRowContext(ctx, getURLFeeds, url)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
SELECT f.id, f.created_at, f.updated_at, f.name, f.url, f.user_id, f.last_fetched_at FROM feeds f
JOIN users u ON f.user_id = u.id
WHERE u.name = $1
`
